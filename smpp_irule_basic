...
when CLIENT_DATA {
    ...
    while { [string length $incoming_buf] >= 16 } {
        ...
        call smpp_log_debug "is_request_msg = ($is_request_msg); command_length = ($command_length); command_id = (0x[format %x $command_id]); command_name = ($static::smpp_command_map($command_id)); command_status = ($command_status); sequence_number = ($sequence_number)" 
